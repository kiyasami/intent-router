...
export function updateCentroid(oldCentroid, oldCount, newVec) {
...
function resolveDimension(dimension, embedOptions) {
...
export function serializeProfile(profile) {
    ...
    const centroids = profile.centroids
        ? Object.fromEntries(Object.entries(profile.centroids).map(([commandId, vec]) => [
            commandId,
...
export function bumpAffinity(args) {
...
export class ProfileManager {
    ...
    async loadProfile(userId, options) {
        ...
        const validatedProfile = this.validateProfileDimensions(clonedProfile, options?.dimension);
        if (validatedProfile) {
    ...
    validateProfileDimensions(profile, dimension = DEFAULT_DIM) {
        if (!profile)
            return {};
        if (profile.centroids) {
            for (const [commandId, vec] of Object.entries(profile.centroids)) {
                if (vec.length !== dimension) {
                    throw new Error(`Profile centroid dimension mismatch for command "${commandId}". Expected ${dimension}, got ${vec.length}.`);
                }
            }
        }
        return profile;
    ...
    importProfile(userId, profile, options) {
        const validated = this.validateProfileDimensions(cloneProfile(profile), options?.dimension);
        this.store.setLocalProfile(userId, validated);
        return validated;
    }
    setLocal(userId, profile, options) {
        this.store.setLocalProfile(userId, this.validateProfileDimensions(profile, options?.dimension));
    }
    resetLocal(userId) {
        this.store.clearLocalProfile?.(userId);
    }
    learnLocal(args, options = {}) {
...
export class LocalStorageProfileStore {
    getLocalProfile(userId) {
        ...
        if (!raw)
            return undefined;
        return cloneProfile(JSON.parse(raw));
    }
    setLocalProfile(userId, profile) {
        localStorage.setItem(`intent-router:${userId}`, JSON.stringify(serializeProfile(profile)));
    }
